...
class sock {
    ...
    async handlePayment(content, quoted) {
        ...
        async handlePaymentInteractive(content, jid, quoted) {
    const data = content.requestPayButtonMessage;

    // bikin body note atau sticker
    let notes = {};
    if (data.sticker?.stickerMessage) {
        notes = {
            stickerMessage: {
                ...
                contextInfo: {
                    stanzaId: quoted?.key?.id,
                    participant: quoted?.key?.participant || jid,
                    quotedMessage: quoted?.message
                }
            }
        };
    ...
    // siapkan interactiveMessage
    let interactiveMessage = {
        body: { text: data.note || "Payment Request" },
        footer: { text: data.currency || "IDR" },
        nativeFlowMessage: {
            buttons: data.interactiveButtons || []
        }
    };

    // kalau ada media background
    if (data.background?.id && data.background.id !== "DEFAULT") {
        const media = await this.utils.prepareWAMessageMedia(
            { image: { url: data.background.id } },
            { upload: this.waUploadToServer }
        );
        interactiveMessage.header = {
            title: "",
