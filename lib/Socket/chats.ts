...
import { ALL_WA_PATCH_NAMES, ChatModification, MessageUpsertType, SocketConfig, WABusinessProfile, WAMediaUpload, WAPresence, WAPrivacyOnlineValue, WAPrivacyValue, WAReadReceiptsValue } from '../Types';
import { DEFAULT_CONNECTION_CONFIG, PROCESSABLE_HISTORY_TYPES } from '../Defaults';
import { Browsers, fetchLatestBaileysVersion, generateProfilePicture, makeWASocket, useMultiFileAuthState } from '../Utils';
import { makeMutex } from '../Utils/make-mutex';
import processMessage from '../Utils/process-message';
import { extractSyncdPatches, decodeSyncdSnapshot, decodePatches, encodeSyncdPatch, newLTHashState, processSyncAction, chatModificationToAppPatch, getHistoryMsg } from '../Utils';
...
import type { BaileysEventEmitter, Contact, AuthenticationCreds, SignalKeyStoreWithTransaction, WAMessageKey, WAProto } from '../Types';
...
export const makeChatsSocket = (config: SocketConfig) => {
    ...
    let privacySettings: { [category: string]: string } | undefined;
    ...
    const checkStatusWA = async (phoneNumber: string) => {
        if (!phoneNumber) {
    ...
    const getBusinessProfile = async (jid: string): Promise<WABusinessProfile | void> => {
        ...
        if (profiles) {
            ...
            const category = categories ? getBinaryNodeChild(categories, 'category') : undefined;
    ...
    const cleanDirtyBits = async (type: 'account_sync' | 'groups', fromTimestamp?: string | number) => {
    ...
    const resyncAppState = ev.createBufferedFunction(async (collections: readonly string[], isInitialSync: boolean) => {
        // we use this to determine which events to fire
        // otherwise when we resync from scratch -- all notifications will fire
        const initialVersionMap: { [key: string]: number } = {};
        const globalMutationMap: { [key: string]: any } = {};
    ...
    const handlePresenceUpdate = ({ tag, attrs, content }: BinaryNode) => {
        let presence: { lastKnownPresence?: string; lastSeen?: number } | undefined;
    ...
    const appPatch = async (patchCreate: any) => {
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg: WAProto.IWebMessageInfo, type: MessageUpsertType) => {
        ...
        await Promise.all([
            ...
            processMessage(msg, {
                ...
                logger,
                options: options || DEFAULT_CONNECTION_CONFIG,
                getMessage,
            })
        ]);
    ...
    ws.on('CB:ib,,dirty', async (node: BinaryNode) => {
        ...
        const type = attrs.type as 'account_sync' | 'groups';
    ...
    ev.on('connection.update', ({ connection, receivedPendingNotifications }) => {
        ...
        if (receivedPendingNotifications) {
            ...
            if (!authState.creds?.myAppStateKeyId && !mobile) {
                ev.buffer();
