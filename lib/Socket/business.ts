...
import { parseCatalogNode, parseCollectionsNode, parseOrderDetailsNode, uploadingNecessaryImagesOfProduct, toProductNode, parseProductNode } from '../Utils/business';
import { jidNormalizedUser, S_WHATSAPP_NET } from '../WABinary';
import { getBinaryNodeChild } from '../WABinary/generic-utils';
import type { SocketConfig } from '../Types';
import type { GetCatalogOptions, ProductCreate, ProductUpdate } from '../Types';
...
export const makeBusinessSocket = (config: SocketConfig) => {
    ...
    const getCollections = async (jid?: string, limit: number = 51) => {
        jid = jid || authState.creds.me?.id;
        if (!jid) {
            throw new Error('JID is required');
        }
